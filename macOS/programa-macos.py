...
###Obtiene la ip del equipo
    for ifaceName in interfaces():
        ...
        if ifaceName=='en0':
            ...
            print '%s: %s' % ('Mi IP: ',ifaceName.join(addresses))
###Realiza la separacion de la ip por puntos
            ipa=str(ifaceName.join(addresses)).split(".")
    print ipa
    ipa[3]=''
    ip_auto=".".join(ipa)
    print ip_auto
######Realiza el escaneo de equipos de acuerdo a la ip del equipo
    if ip_auto:
        ...
        for ping in range(1, 255):
            ip= ip_auto + str(ping)
            result=subprocess.call(["ping", "-c 1", "-n", "-i 0.5", "-W1", ip])
...
def myip():
    ...
    for ifaceName in interfaces():
        ...
        if ifaceName=='en0':
            #print '%s: %s' % (ifaceName, ', '.join(addresses))
            print '%s: %s' % ('Mi IP: ',ifaceName.join(addresses))
            myip = Label(ventana3,text="Mi IP: "+str(ifaceName.join(addresses)),bg=colorFondo,fg=colorLetra,font=("Helvetica",16))
            myip.pack()
    ventana3.mainloop()
...
def hosts():
    ...
    if checke.get()==1 and checks.get()==0 and ingresatuIP.get():
        ...
        return 0
#Funcion que realiza el escaneo de equipos activos
def eq():
    t_in = datetime.now()
    for ping in range(1, 255):
        ip= ip_auto + str(ping)
...
#Posicion de los elementos graficos de la interfaz
ventana = Tk()
